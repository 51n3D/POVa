...
def main():
    ...
    while(True):
        ...
        if crop_cb.cropping_now:
            ...
            # Draw into tmp_img.
            tmp_img = cv2.rectangle(tmp_img, crop_cb.first_point, crop_cb.second_point, (255, 0, 0), thickness=2)
        ...
        if key == 27:   # 27 this is the Escape key
            ...
            rect = (
                crop_cb.first_point[0], crop_cb.first_point[1],
                crop_cb.second_point[0], crop_cb.second_point[1]
            )
            segment_cb.mask, bg_color_model, fg_color_model = cv2.grabCut(
                segment_cb.img, segment_cb.mask, rect, bg_color_model, fg_color_model, 2
            )
    ...
    if segment_cb:
        ...
        # FILL
        mask = np.logical_or(segment_cb.mask == cv2.GC_FGD, segment_cb.mask == cv2.GC_PR_FGD)
        binary_mask = np.where(mask, 1, 0)
 
        # Add some random foreground noise pixels to binary_mask.
        positions0 = np.random.randint(binary_mask.shape[0] - 1, size=100)
        positions1 = np.random.randint(
        ...
        segment_cb.img[binary_mask.astype(bool)] = (0, 0, 0)
        masked_foreground_image = segment_cb.img
        ...
        distances = cv2.distanceTransform(1 - binary_mask, cv2.DIST_L2, 3)

        outline_20_px = np.uint32(distances) == 20
        cv2.imshow('distance 20', np.uint8(outline_20_px) * 255)
        cv2.waitKey()

        # Compute vertical and horizontal projection of the foreground in binary_mask.ss
        ...
        plt.subplot(1, 2, 1).plot(np.arange(horizontal_projection.shape[0]), horizontal_projection)
        plt.subplot(1, 2, 2).plot(np.arange(vertical_projection.shape[0]), vertical_projection)
